...
var ExitingPersistence = /*#__PURE__*/(0, _react.memo)(function (_ref) {
  ...
  var nextChildren = currentChildren !== children ? childrenToArray(children) : current;
  var missingKeys = getMissingKeys(nextChildren, previous);
  var isSomeChildRemoved = !!missingKeys.size;
  var visibleChildren = !isSomeChildRemoved && currentChildren !== children ? nextChildren : current;
  if (isSomeChildRemoved) {
    visibleChildren = spliceNewElementsIntoPrevious(current, previous);
  }
  if (exitThenEnter) {
    if (exitingChildren.length) {
      visibleChildren = (0, _platformFeatureFlags.fg)('platform-dst-motion-uplift') ? previous : exitingChildren;
    } else {
