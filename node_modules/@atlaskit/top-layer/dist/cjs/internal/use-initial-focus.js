...
var _getFirstFocusable = require("../focus/get-first-focusable");
/**
...
var TEXT_LIKE_INPUT_TYPES = new Set(['text', 'search', 'email', 'url', 'tel', 'password']);
function isTextboxLikeElement(element) {
  ...
  if (tag === 'input') {
    ...
    var type = ((_element$getAttribute = element.getAttribute('type')) !== null && _element$getAttribute !== void 0 ? _element$getAttribute : 'text').toLowerCase();
    return TEXT_LIKE_INPUT_TYPES.has(type);
  }
  return false;
}
function isComboboxControllingPopup(_ref) {
  var _active$getAttribute;
  ...
  var idRefs = (_active$getAttribute = active.getAttribute('aria-controls')) !== null && _active$getAttribute !== void 0 ? _active$getAttribute : active.getAttribute('aria-owns');
  if (!idRefs) {
    return false;
  }
  ...
  // host we render).
  var referencedIds = idRefs.split(/\s+/).filter(function (id) {
    return id !== '';
  });
  return referencedIds.some(function (id) {
    var referenced = ownerDocument.getElementById(id);
    if (!referenced) {
      return false;
    }
    return referenced === container || referenced.contains(container) || container.contains(referenced);
  });
}
function getInitialFocusTarget(_ref2) {
  var container = _ref2.container,
    role = _ref2.role;
  if (role === 'dialog' || role === 'alertdialog') {
    // Prefer element with the native HTML autofocus attribute.
    var autofocusEl = container.querySelector('[autofocus]');
    if (autofocusEl) {
      return autofocusEl;
    }
    // Fall back to first focusable element
    return (0, _getFirstFocusable.getFirstFocusable)({
      container: container
    });
  }
  ...
  if (role === 'listbox') {
    ...
    var selectedOption = container.querySelector('[role="option"][aria-selected="true"]');
    if (selectedOption) {
      return selectedOption;
    }
    var firstOption = container.querySelector('[role="option"]');
    if (firstOption) {
      return firstOption;
    }
 ...
 */
function useInitialFocus(_ref3) {
  var elementRef = _ref3.elementRef,
    phase = _ref3.phase,
    role = _ref3.role;
  ...
  var prevPhaseRef = (0, _react.useRef)('closed');
  (0, _react.useEffect)(function () {
    var _getInitialFocusTarge;
    var prevPhase = prevPhaseRef.current;
    ...
    (_getInitialFocusTarge = getInitialFocusTarget({
      container: element,
      role: role
    })) === null || _getInitialFocusTarge === void 0 || _getInitialFocusTarge.focus();
