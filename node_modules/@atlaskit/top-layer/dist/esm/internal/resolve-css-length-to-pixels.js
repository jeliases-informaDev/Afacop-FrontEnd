var elementCaches = new WeakMap();
function canCacheResolvedLength(value) {
...
function getCachedResolvedLength(_ref) {
  var _cacheForElement$get;
  var element = _ref.element,
    value = _ref.value;
  if (!canCacheResolvedLength(value)) {
    return null;
  }
  var cacheForElement = elementCaches.get(element);
  return (_cacheForElement$get = cacheForElement === null || cacheForElement === void 0 ? void 0 : cacheForElement.get(value)) !== null && _cacheForElement$get !== void 0 ? _cacheForElement$get : null;
}
function cacheResolvedLength(_ref2) {
  var _elementCaches$get;
  ...
  var cacheForElement = (_elementCaches$get = elementCaches.get(element)) !== null && _elementCaches$get !== void 0 ? _elementCaches$get : new Map();
 ...
 */
export function resolveCssLengthToPixels(_ref3) {
  ...
  var cached = getCachedResolvedLength({
    element: container,
    value: value
  });
  if (cached !== null) {
  ...
  var probe = container.ownerDocument.createElement('div');
  ...
  var resolved = parseFloat(getComputedStyle(probe).marginLeft);
  container.removeChild(probe);
  var result = Number.isFinite(resolved) ? resolved : 0;
  cacheResolvedLength({
    element: container,
    value: value,
    result: result
