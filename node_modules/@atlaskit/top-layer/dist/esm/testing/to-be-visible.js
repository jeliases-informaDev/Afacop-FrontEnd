 ...
 */
function nearestTopLayerState(element) {
  if (!element) {
    return null;
  }
...
function getNearestTopLayerType(element) {
  if (!element.closest) {
...
expect.extend({
  toBeVisible: function toBeVisible(element) {
    var originalResult = originalToBeVisible.call(this, element);
    var topLayerState = nearestTopLayerState(element);
      ...
      if (topLayerState === 'closed') {
        return {
          ...
          message: function message() {
            return "expected element to be visible, but it is inside a closed ".concat(getNearestTopLayerType(element));
          }
        };
      }
    ...
    if (topLayerState === 'open') {
      return {
        pass: true,
        message: function message() {
          return "expected element not to be visible, but it is inside an open ".concat(getNearestTopLayerType(element));
        }
      };
    }
    return originalResult;
