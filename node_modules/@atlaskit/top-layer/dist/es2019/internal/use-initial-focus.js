...
function isTextboxLikeElement(element) {
  ...
  if (tag === 'input') {
    ...
    const type = ((_element$getAttribute = element.getAttribute('type')) !== null && _element$getAttribute !== void 0 ? _element$getAttribute : 'text').toLowerCase();
    return TEXT_LIKE_INPUT_TYPES.has(type);
  }
  return false;
}
function isComboboxControllingPopup({
  container
}) {
  ...
  const idRefs = (_active$getAttribute = active.getAttribute('aria-controls')) !== null && _active$getAttribute !== void 0 ? _active$getAttribute : active.getAttribute('aria-owns');
  if (!idRefs) {
    return false;
  }
...
export function useInitialFocus({
  ...
  useEffect(() => {
    ...
    (_getInitialFocusTarge = getInitialFocusTarget({
      container: element,
      role
    })) === null || _getInitialFocusTarge === void 0 ? void 0 : _getInitialFocusTarge.focus();
