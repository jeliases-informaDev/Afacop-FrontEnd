...
function createToggleEvent({
  ...
  if (typeof globalThis.ToggleEvent === 'function') {
    ...
    const ToggleEventCtor = globalThis.ToggleEvent;
...
function clearOpacityOnOpenToggle({
  element
}) {
...
function findInvokerForPopover({
  popover
}) {
...
function isInChainTo({
  ...
  if (target instanceof HTMLElement && target.hasAttribute('popover')) {
    const invoker = findInvokerForPopover({
      popover: target
    });
...
function hidePopoversInStackNotInChainTo({
  ...
  const stopIndex = snapshot.findIndex(popover => isInChainTo({
    candidate: popover,
    target
  }));
...
function handleDocumentPointerup(event) {
  ...
  const target = event.target instanceof Node ? event.target : null;
  const ancestor = nearestOpenPopover({
    target
  });
  ...
  const ancestorMode = ancestor ? getPopoverMode({
    element: ancestor
  }) : null;
...
function handleDocumentKeydown(event) {
  ...
  const topHint = topmostConnected({
    stack: openHintStack
  });
  if (topHint) {
    topHint.hidePopover();
  ...
  const topAuto = topmostConnected({
    stack: openAutoStack
  });
  if (topAuto) {
    topAuto.hidePopover();
...
if (typeof HTMLElement !== 'undefined') {
  ...
  if (!HTMLElement.prototype.showPopover) {
    HTMLElement.prototype.showPopover = function showPopover() {
      if (!this.hasAttribute('popover')) {
        throw new DOMException(`Failed to execute 'showPopover' on 'HTMLElement': ${describeElement({
          element: this
  ...
  if (!HTMLElement.prototype.hidePopover) {
    HTMLElement.prototype.hidePopover = function hidePopover() {
      if (!this.hasAttribute('popover')) {
        throw new DOMException(`Failed to execute 'hidePopover' on 'HTMLElement': ${describeElement({
          element: this
        })} is not a popover element. Ensure the element has a 'popover' attribute (e.g. <div popover>).`, 'NotSupportedError');
      }
  ...
  if (typeof document !== 'undefined' && !document.__atTopLayerLightDismiss) {
    ...
    const pointerdownAdapter = event => handleDocumentPointerdown(event);
    const pointerupAdapter = event => handleDocumentPointerup(event);
    bind(document, {
      type: 'pointerdown',
...
if (typeof HTMLDialogElement !== 'undefined') {
  ...
  if (typeof proto.showModal !== 'function') {
    proto.showModal = function showModal() {
      if (!this.isConnected) {
        throw new DOMException(`Failed to execute 'showModal' on 'HTMLDialogElement': ${describeElement({
          element: this
