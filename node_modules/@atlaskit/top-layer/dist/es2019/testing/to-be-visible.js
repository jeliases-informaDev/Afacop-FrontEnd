...
expect.extend({
  toBeVisible(element) {
    const originalResult = originalToBeVisible.call(this, element);
    ...
    if (originalResult.pass) {
      ...
      if (topLayerState === 'closed') {
        return {
          pass: false,
          message: () => `expected element to be visible, but it is inside a closed ${getNearestTopLayerType(element)}`
        };
      }
    ...
    if (topLayerState === 'open') {
      return {
        pass: true,
        message: () => `expected element not to be visible, but it is inside an open ${getNearestTopLayerType(element)}`
      };
    }
    return originalResult;
  }
